...
filter_df <- function (data_in) {
	...
	#data_in <- data_in[!grepl('Initial',data_in$OpportunityType),]
	#data_in <- data_in[!grepl('Upsell',data_in$OpportunityType),]
	data_in <- data_in[!grepl('Renewal',data_in$OpportunityType),]
	data_in <- data_in[!grepl('Cancellation',data_in$OpportunityType),]
	data_in <- data_in[!grepl('Credit',data_in$OpportunityType),]
	data_in <- data_in[!grepl('0',data_in$OpportunityType),]
	data_in <- data_in[!grepl('Swap',data_in$OpportunityType),]
	
	names(data_in)[names(data_in) == 'OpportunityId'] <- "opid"
	...
	data_in <- data_in[data_in$final_day==data_in$tstop,] 

	#### This will cause sdata_op_history_df_COX.csv to HAVE NO DATA
	data_in <- data_in[data_in$final_day==data_in$tstart,] 

	data_in <- data_in[data_in$tstop!=max(data_in$tstop),] ## remove max tstop = 17016 ... not sure why it is that number ... only about 20+ 
	data_in$max_tstop <- NULL  # Remove column
...
# Load data
##################
input_sdata <- read.csv(file="../../output/sdata_all_history_RSF_20161004_ORIG.csv", header=TRUE, sep=",",stringsAsFactors=FALSE)
account_df <- read.csv(file="../../output/account_df.csv",header=TRUE,sep=",",stringsAsFactors=FALSE)

cat(sprintf('After Initial Read         ... No of Accounts: %4s\n',length(unique(input_sdata$opid))) )
...
#input_sdata <- input_sdata[input_sdata$Op_CreatedDate > '2015-04-05 00:00:00',]  # Day that CloseDate / Amount changes were logged in op history
...
#output_var$n1 = c('s1','s2','s3','s4','s5','stageback') ## All significant ... results aren't as good
#output_var$n1 = c('tstop','won','OpportunityType','Product_Line__c','s1','s2','s3','s4','s5','NaicsCode','stageback') ## All significant ... results aren't as good
#output_var$n1 = c('opid','Op_CreatedDate','tstop','won','lost','OpportunityType','Product_Line__c','NaicsCode','NumberOfEmployees','LeadSource','month_created','s1','s2','s3','s4','s5','stageback','close_change','close_push','close_pullin','amount_change','amount_up','amount_down','amount_per')
output_var$n1 = c('opid','Op_CreatedDate','tstop','won','lost','OpportunityType','Product_Line__c','NaicsCode','NumberOfEmployees','LeadSource','month_created','s1','s2','s3','s4','stageback','close_change','close_push','close_pullin','amount_change','amount_up','amount_down','amount_per')
...
###################################
###################################
rfsrc_pbc <- rfsrc(Surv(tstop, won) ~ ., data = pbc.train[,!(names(pbc.train) %in% c('opid','Op_CreatedDate','lost'))] , 
					forest = T,   # required to use vimp functions
					tree.err = T, # required for plotting tree error
					nsplit = 10, na.action = "na.impute")

timeROC = c(10,30,median(pbc.train$tstop),90,120,150,180,210)
...
p7 <- plot(gg_error(rfsrc_pbc)) + coord_cartesian(ylim = c(0.09, 0.31))
print(p7)
...
if (VERIFY_TRAINING_SET == FALSE) {
	rfsrc_pbc_test <- predict(rfsrc_pbc, newdata = pbc.test[,!(names(pbc.test) %in% c('opid','Op_CreatedDate','lost'))],
	                          	na.action = "na.impute")
	final_pred <- pbc.test[,c("opid","Op_CreatedDate","tstop",'won','lost')]
} else {
	rfsrc_pbc_test <- predict(rfsrc_pbc, newdata = pbc.train[,!(names(pbc.train) %in% c('opid','Op_CreatedDate','lost'))],
   		                       	na.action = "na.impute")
	final_pred <- pbc.train[,c("opid","Op_CreatedDate","tstop",'won','lost')] 
}
