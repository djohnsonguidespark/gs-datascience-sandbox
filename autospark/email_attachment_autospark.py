...
filename = './output/autospark_weekly_' + datetime.now().strftime('%Y%m%d') +'.xlsx'
...
receiver = ['Devon Johnson <djohnson@guidespark.com>',
			'Arjun Ramamurthy <aramamurthy@guidespark.com>'] 
#receiver = ['Devon Johnson <djohnson@guidespark.com>'] 
...
###############################################
msg = MIMEMultipart()
msg['Subject'] = 'AutoSpark Weekly ... ' + datetime.now().strftime('%Y-%m-%d')  
msg['From'] = sender 
msg['To'] = ','.join(receiver) # join allows for multiple emails 
text = "AutoSpark Weekly Report"
